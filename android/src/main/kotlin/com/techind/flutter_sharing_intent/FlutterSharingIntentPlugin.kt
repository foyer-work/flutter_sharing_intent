...
class FlutterSharingIntentPlugin : FlutterPlugin, ActivityAware, MethodCallHandler,
    ...
    companion object {
        private var TAG: String = FlutterSharingIntentPlugin::class.java.name

        @Volatile
        private var instance: FlutterSharingIntentPlugin? = null

        fun getInstance(): FlutterSharingIntentPlugin {
            return instance ?: synchronized(this) {
                instance ?: FlutterSharingIntentPlugin().also { instance = it }
            }
        }

        fun handleIncomingIntent(intent: Intent, initial: Boolean, context: Context) {
            val plugin = getInstance()
            val binaryMessenger = FlutterEngine(context).dartExecutor.binaryMessenger
            plugin.initFromIntent(context, binaryMessenger)
            plugin.handleIntent(intent, initial)
        }
    }
    ...
    private fun initFromIntent(context: Context, binaryMessenger: BinaryMessenger) {
    ...
    private fun handleIntent(intent: Intent, initial: Boolean) {
        ...
        if ((intentFlags and Intent.FLAG_ACTIVITY_LAUNCHED_FROM_HISTORY) == 0) {
            Log.w(
                TAG,
                "pluginLog handleIntent ==>> ${intent.action}, ${intent.type}, ${eventSinkSharing}"
            )
            val activityType = intent.getStringExtra("activityType") ?: "ask"
            val isSaveIntent = activityType == "save"
            val action = if (isSaveIntent) ActionType.SAVE else ActionType.ASK
            when {
                (intent.type?.startsWith("text") != true)
                    ...
                    val value = getSharingUris(intent, action)
                    if (initial) initialSharing = value
                    latestSharing = value
                    Log.w(TAG, "pluginLog handleIntent Image/Video ==>> $value ${instance}")
                    eventSinkSharing?.success(value?.toString())
                }

                (intent.type == null || intent.type?.startsWith("text") == true)
                        && (intent.action == Intent.ACTION_SEND || intent.action == Intent.ACTION_SEND_MULTIPLE || intent.action == Intent.ACTION_PROCESS_TEXT) -> { // Sharing text

                    val value = getSharingText(intent, action) ?: getSharingUris(intent, action)
                    if (initial) initialSharing = value
                    latestSharing = value
                    Log.w(TAG, "pluginLog handleIntent Text ==>> $value ${instance}")
                    eventSinkSharing?.success(value?.toString())
                ...
                intent.action == Intent.ACTION_VIEW -> { // Opening URL
                    ...
                    if (initial) initialSharing = value
                    latestSharing = value
                    Log.w(TAG, "pluginLog handleIntent Url ==>> $value $instance")
                    eventSinkSharing?.success(value?.toString())
                }
            }
        }
    ...
    private fun getSharingUris(intent: Intent?, action: ActionType?): JSONArray? {
                ...
                if (path != null) {
                    ...
                    JSONArray().put(
                        JSONObject()
                            ...
                            .put("action", action?.name ?: ActionType.ASK)
    ...
    private fun getSharingText(intent: Intent?, action: ActionType?): JSONArray? {
        if (intent == null) return null
